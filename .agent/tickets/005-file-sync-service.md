...
- Added a regression test to ensure `TransferMode::Direct` paths omit the `-z` flag and expanded the CLI trycmd snapshot suite with a `content` scope dry-run to cover both file operations.
